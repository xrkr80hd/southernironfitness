...
type ExerciseDraft = { exercise: string; sets: string; reps: string; weight: string; rest: string; tempo: string; notes: string };
...
type WorkoutLog = { id: string; program_item_id: string; client_id: string; completed_at: string; actual_sets: number | null; actual_reps: string | null; actual_weight: string | null; client_notes: string | null };
...
type CopiedSlot = { start: string; end: string; note: string | null };

const money = (cents: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(cents / 100);
const toCents = (value: FormDataEntryValue | null) => Math.round(Number(value || 0) * 100);
const blankExercise = (): ExerciseDraft => ({ exercise: "", sets: "", reps: "", weight: "", rest: "", tempo: "", notes: "" });
...
export default function LiftLabAdmin() {
  ...
  const [copiedSlots, setCopiedSlots] = useState<CopiedSlot[]>([]);
  const [message, setMessage] = useState("");
  const [selectedProgramId, setSelectedProgramId] = useState("");
  const [workoutDay, setWorkoutDay] = useState("Day 1");
  const [workoutDate, setWorkoutDate] = useState("");
  const [exerciseDraft, setExerciseDraft] = useState<ExerciseDraft>(blankExercise());
    ...
    try {
      ...
      if (currentRole === "admin") {
        const [programResult, itemResult, paymentResult, settingsResult, workoutResult, preferenceResult, photoResult, signupNotificationResult, workflowNotificationResult] = await Promise.all([
          ...
          supabase.from("lift_lab_workout_logs").select("id, program_item_id, client_id, completed_at, actual_sets, actual_reps, actual_weight, client_notes").order("completed_at", { ascending: false }),
  ...
  async function createAvailability(event: FormEvent<HTMLFormElement>) {
    ...
    if (role !== "admin") return;
    const form = new FormData(event.currentTarget);
    const startsAt = new Date(`${selectedCalendarDate}T${String(form.get("startTime"))}`);
    const endsAt = new Date(`${selectedCalendarDate}T${String(form.get("endTime"))}`);
    if (endsAt <= startsAt) { setMessage("The end time must be after the start time."); return; }
  ...
  function copySelectedDaySlots() {
    const source = slots.filter(slot => slot.status === "open" && dateKey(new Date(slot.starts_at)) === selectedCalendarDate).map(slot => ({ start: new Date(slot.starts_at).toTimeString().slice(0, 5), end: new Date(slot.ends_at).toTimeString().slice(0, 5), note: slot.note }));
    setCopiedSlots(source);
    setMessage(source.length ? `${source.length} open time${source.length === 1 ? "" : "s"} copied. Tap another day and paste.` : "This day has no open times to copy.");
  }

  async function pasteSlotsToSelectedDay() {
    if (role !== "admin" || !copiedSlots.length) { setMessage("Copy a day with open times first."); return; }
    const rows = copiedSlots.map(slot => ({ trainer_id: userId, starts_at: new Date(`${selectedCalendarDate}T${slot.start}`).toISOString(), ends_at: new Date(`${selectedCalendarDate}T${slot.end}`).toISOString(), status: "open", note: slot.note }));
    const { error } = await createClient().from("lift_lab_availability").insert(rows);
    setMessage(error ? error.message : `${rows.length} copied time${rows.length === 1 ? "" : "s"} pasted onto ${new Date(`${selectedCalendarDate}T12:00`).toLocaleDateString()}.`);
    if (!error) await load();
  }
  ...
  async function createTimeOff(event: FormEvent<HTMLFormElement>) {
    ...
    if (role !== "admin") return;
    const form = new FormData(event.currentTarget);
    const date = String(form.get("offDate"));
    const fullDay = form.get("fullDay") === "on";
    const startTime = fullDay ? "00:00" : String(form.get("offStart"));
    const endTime = fullDay ? "23:59" : String(form.get("offEnd"));
    if (!date || !startTime || !endTime) { setMessage("Choose the date and time Madie is unavailable."); return; }
    const startsAt = new Date(`${date}T${startTime}`);
    const endsAt = new Date(`${date}T${endTime}`);
    if (endsAt <= startsAt) { setMessage("The unavailable end time must be after the start time."); return; }
    const supabase = createClient();
    const overlapping = slots.filter(slot => slot.status === "open" && new Date(slot.starts_at) < endsAt && new Date(slot.ends_at) > startsAt).map(slot => slot.id);
    if (overlapping.length) {
      const blocked = await supabase.from("lift_lab_availability").update({ status: "blocked", updated_at: new Date().toISOString() }).in("id", overlapping);
      if (blocked.error) { setMessage(blocked.error.message); return; }
    }
    const note = `TIME_OFF:${fullDay ? "Full day off" : String(form.get("offNote") || "Unavailable")}`;
    const { error } = await supabase.from("lift_lab_availability").insert({ trainer_id: userId, starts_at: startsAt.toISOString(), ends_at: endsAt.toISOString(), status: "blocked", note });
    setMessage(error ? error.message : fullDay ? "Day off saved. Customers cannot request a time that day." : "Time off saved. Overlapping open times were blocked.");
    if (!error) { event.currentTarget.reset(); await load(); }
  }
  ...
  function updateExercise(field: keyof ExerciseDraft, value: string) {
    setExerciseDraft(current => ({ ...current, [field]: value }));
  }

  function startNextWorkoutDay() {
    const numberedDays = workoutDays.map(day => Number(day.match(/day\s+(\d+)/i)?.[1] || 0));
    const nextDay = Math.max(0, ...numberedDays) + 1;
    setWorkoutDay(`Day ${nextDay}`);
    setWorkoutDate("");
    setExerciseDraft(blankExercise());
    setMessage(`Day ${nextDay} is ready. Add the first exercise.`);
  }
  ...
  async function addWorkoutExercise(event: FormEvent<HTMLFormElement>) {
    ...
    if (!program) { setMessage("Choose the client program for this workout."); return; }
    if (!exerciseDraft.exercise.trim()) { setMessage("Enter an exercise before saving."); return; }
    const dayItems = programItems.filter(item => item.program_id === program.id && (item.day_label || "Flexible day") === (workoutDay.trim() || "Flexible day"));
    const { error } = await createClient().from("lift_lab_program_items").insert({ program_id: program.id, trainer_id: userId, client_id: program.client_id, scheduled_for: workoutDate || null, day_label: workoutDay.trim() || "Flexible day", sort_order: dayItems.length, exercise_name: exerciseDraft.exercise.trim(), target_sets: Number(exerciseDraft.sets) || null, target_reps: exerciseDraft.reps.trim() || null, target_weight: exerciseDraft.weight.trim() || null, rest_seconds: Number(exerciseDraft.rest) || null, tempo: exerciseDraft.tempo.trim() || null, trainer_notes: exerciseDraft.notes.trim() || null });
    setMessage(error ? error.message : `${exerciseDraft.exercise.trim()} saved to ${workoutDay.trim() || "the workout"}. Add the next exercise when ready.`);
    if (!error) { setExerciseDraft(blankExercise()); await load(); }
  }
  ...
  const selectedClient = selectedProgram ? clients.find(client => client.id === selectedProgram.client_id) ?? null : null;
  const selectedItems = selectedProgram ? programItems.filter(item => item.program_id === selectedProgram.id) : [];
  const workoutDays = Array.from(new Set(selectedItems.map(item => item.day_label || "Flexible day")));
  ...
  return (
    <section className="lift-lab-admin lift-suite">
      ...
      <details className="lift-menu-accordion"><summary>Dashboard menu · {tab}</summary><nav className="lift-suite__tabs" aria-label="Dashboard sections">{tabs.map(item => <button key={item} className={tab === item ? "is-active" : ""} onClick={() => setTab(item)}>{item}</button>)}</nav></details>
      ...
      {tab === "calendar" ? <div className="lift-suite__stack">
        <section className="lift-lab-card lift-suite__calendar-card"><div className="lift-suite__calendar-head"><button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}>←</button><h2>{month.toLocaleDateString("en-US", { month: "long", year: "numeric" })}</h2><button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}>→</button></div><p className="lift-calendar-hint">Tap a day to manage its schedule.</p><div className="lift-suite__weekdays">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(day => <strong key={day}>{day}</strong>)}</div><div className="lift-suite__calendar">{calendarDays.map(day => { const key = dateKey(day); const daySlots = slots.filter(item => dateKey(new Date(item.starts_at)) === key); const dayAppointments = appointments.filter(item => item.availability && dateKey(new Date(item.availability.starts_at)) === key && !["declined", "cancelled"].includes(item.status)); return <button type="button" key={day.toISOString()} onClick={() => setSelectedCalendarDate(key)} className={`${day.getMonth() === month.getMonth() ? "" : "is-muted"} ${selectedCalendarDate === key ? "is-selected" : ""}`} aria-label={`Manage ${day.toLocaleDateString()}`}><span>{day.getDate()}</span>{daySlots.length ? <small>{daySlots.length} open</small> : null}{dayAppointments.length ? <b>{dayAppointments.length} booked</b> : null}</button>; })}</div></section>
        {trainer ? <section className="lift-calendar-editor"><div className="lift-calendar-selected"><span>Selected day</span><strong>{new Date(`${selectedCalendarDate}T12:00`).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</strong><div className="lift-master-actions"><button className="button button--outline" type="button" onClick={copySelectedDaySlots}>Copy This Day’s Times</button><button className="button" type="button" disabled={!copiedSlots.length} onClick={pasteSlotsToSelectedDay}>Paste {copiedSlots.length || ""} Time{copiedSlots.length === 1 ? "" : "s"} Here</button></div></div><details className="lift-accordion" open><summary><span>Add custom time</span><small>{new Date(`${selectedCalendarDate}T12:00`).toLocaleDateString()}</small></summary><div className="lift-accordion__content"><form className="lift-lab-form" onSubmit={createAvailability}><div className="lift-lab-form-row"><label>Starts<input name="startTime" type="time" required /></label><label>Ends<input name="endTime" type="time" required /></label></div><label>Repeat weekly<input name="repeatWeeks" type="number" min="1" max="52" defaultValue="1" /></label><label>Private note<input name="note" /></label><button className="button">Add Time Slot</button></form></div></details><details className="lift-accordion"><summary><span>Mark time off</span><small>Full or partial day</small></summary><div className="lift-accordion__content"><form className="lift-lab-form" onSubmit={createTimeOff}><label>Date<input name="offDate" type="date" required value={selectedCalendarDate} onChange={event => setSelectedCalendarDate(event.target.value)} /></label><label className="lift-lab-disclaimer"><input name="fullDay" type="checkbox" /><span><strong>Entire day off</strong><small>Leave the times below blank.</small></span></label><div className="lift-lab-form-row"><label>From <span>(for partial day)</span><input name="offStart" type="time" /></label><label>Until <span>(for partial day)</span><input name="offEnd" type="time" /></label></div><label>Private note <span>(optional)</span><input name="offNote" placeholder="Appointment, vacation, unavailable…" /></label><button className="button button--outline">Save Time Off</button></form></div></details></section> : null}
        {trainer ? <section className="lift-lab-card"><h3>Manage upcoming times</h3><div className="lift-suite__list">{slots.filter(slot => new Date(slot.ends_at) >= new Date()).slice(0, 60).map(slot => { const timeOff = slot.note?.startsWith("TIME_OFF:"); return <article key={slot.id}><div><strong>{timeOff ? "Time off · " : ""}{new Date(slot.starts_at).toLocaleString()}</strong><span>Ends {new Date(slot.ends_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}{slot.note ? ` · ${slot.note.replace("TIME_OFF:", "")}` : ""}</span></div><span className={`lift-lab-status lift-lab-status--${slot.status}`}>{timeOff ? "off" : slot.status}</span><div className="lift-lab-request-actions">{!timeOff && slot.status !== "open" ? <button onClick={() => setAvailabilityStatus(slot, "open")}>Open</button> : null}{slot.status !== "blocked" ? <button onClick={() => setAvailabilityStatus(slot, "blocked")}>Block</button> : null}{slot.status !== "closed" ? <button onClick={() => setAvailabilityStatus(slot, "closed")}>{timeOff ? "Remove time off" : "Close"}</button> : null}</div></article>; })}</div></section> : null}
      ...
      {tab === "programs" && trainer ? <div className="lift-suite__stack lift-admin-accordions">
        <details className="lift-accordion"><summary><span>Create a client program</span><small>Choose the trainee and establish their plan</small></summary><div className="lift-accordion__content"><form className="lift-lab-form" onSubmit={createProgram}><label>Client<select name="clientId" required><option value="">Choose a client</option>{clients.filter(item => item.approval_status === "approved").map(client => <option key={client.id} value={client.id}>{client.full_name}{client.account_number ? ` · ${client.account_number}` : ""}</option>)}</select></label><label>Program name<input name="title" required /></label><label>Goal<input name="goal" placeholder="Build strength and consistency" /></label><label>Sessions each week<input name="frequency" type="number" min="1" max="14" defaultValue="3" /></label><label>Program description<textarea name="description" /></label><div className="lift-lab-form-row"><label>Starts<input name="startsOn" type="date" /></label><label>Ends<input name="endsOn" type="date" /></label></div><button className="button">Create Program</button></form></div></details>
        <details className="lift-accordion" open><summary><span>Build a trainee workout</span><small>Select their card, then save one exercise at a time</small></summary><div className="lift-accordion__content"><label className="lift-standalone-label">Trainee program<select value={selectedProgramId} onChange={event => { setSelectedProgramId(event.target.value); setWorkoutDay("Day 1"); }}><option value="">Choose a trainee and program</option>{programs.map(program => <option key={program.id} value={program.id}>{program.client?.full_name} · {program.title}</option>)}</select></label>{selectedProgram && selectedClient ? <article className="lift-trainee-card"><header><div><p className="eyebrow">Trainee prescription</p><h2>{selectedClient.full_name}</h2><span>{selectedClient.account_number || "Account number pending"} · {selectedProgram.title}</span></div><button className="lift-preview__small" onClick={() => setProgramFrozen(selectedProgram.client_id, selectedProgram.status !== "frozen")}>{selectedProgram.status === "frozen" ? "Unfreeze Program" : "Freeze Program"}</button></header><div className="lift-workout-day-heading"><div className="lift-lab-form-row lift-workout-day-controls"><label>Workout day<input value={workoutDay} onChange={event => setWorkoutDay(event.target.value)} placeholder="Day 1 · Chest" /></label><label>Date <span>(optional)</span><input value={workoutDate} onChange={event => setWorkoutDate(event.target.value)} type="date" /></label></div><button type="button" className="button button--outline" onClick={startNextWorkoutDay}>+ Add Next Day</button></div><details className="lift-accordion lift-accordion--nested" open><summary><span>Add a workout exercise</span><small>{workoutDay || "Selected day"}</small></summary><div className="lift-accordion__content"><form className="lift-lab-form" onSubmit={addWorkoutExercise}><label>Exercise<input value={exerciseDraft.exercise} onChange={event => updateExercise("exercise", event.target.value)} placeholder="Bench press" required /></label><div className="lift-lab-form-row"><label>Sets<input value={exerciseDraft.sets} onChange={event => updateExercise("sets", event.target.value)} type="number" min="1" placeholder="3" /></label><label>Reps<input value={exerciseDraft.reps} onChange={event => updateExercise("reps", event.target.value)} placeholder="8-10" /></label></div><div className="lift-lab-form-row"><label>Weight target<input value={exerciseDraft.weight} onChange={event => updateExercise("weight", event.target.value)} placeholder="135 lb or client appropriate" /></label><label>Rest seconds<input value={exerciseDraft.rest} onChange={event => updateExercise("rest", event.target.value)} type="number" min="0" placeholder="60" /></label></div><label>Tempo<input value={exerciseDraft.tempo} onChange={event => updateExercise("tempo", event.target.value)} placeholder="Controlled" /></label><label>Madie’s instructions<textarea value={exerciseDraft.notes} onChange={event => updateExercise("notes", event.target.value)} rows={4} placeholder="Form cues, substitutions, and intensity notes" /></label><button className="button">Save Exercise to {workoutDay || "Workout"}</button></form></div></details><div className="lift-workout-days">{workoutDays.length ? workoutDays.map(day => { const dayExercises = selectedItems.filter(item => (item.day_label || "Flexible day") === day); return <details className="lift-accordion lift-accordion--day" key={day}><summary><span>{day}</span><small>{dayExercises.length} exercise{dayExercises.length === 1 ? "" : "s"}</small></summary><div className="lift-accordion__content"><div className="lift-suite__list">{dayExercises.map(item => { const log = workoutLogs.find(entry => entry.program_item_id === item.id); return <article key={item.id}><div><strong>{item.exercise_name}</strong><span>{item.target_sets || "—"} sets × {item.target_reps || "—"}{item.target_weight ? ` · ${item.target_weight}` : ""}{item.rest_seconds != null ? ` · ${item.rest_seconds}s rest` : ""}{item.tempo ? ` · ${item.tempo} tempo` : ""}</span>{log ? <p className="lift-client-note"><strong>Trainee log:</strong> {log.actual_sets ?? "—"} sets × {log.actual_reps || "—"}{log.actual_weight ? ` · ${log.actual_weight}` : ""}<br />{log.client_notes || "No trainee note."}</p> : <small>Not logged yet</small>}</div><span className={`lift-lab-status lift-lab-status--${log ? "completed" : "pending"}`}>{log ? "completed" : "assigned"}</span></article>})}</div></div></details>; }) : <p className="lift-lab-card__intro">No exercises saved yet. Save the first exercise above, then repeat until Day 1 is complete. Use Add Next Day for Day 2, Day 3, Day 5, or as many days as the program needs.</p>}</div></article> : <div className="lift-lab-empty"><strong>Choose a trainee.</strong><p>Their program card, workout days, exercises, and completed logs will appear here.</p></div>}</div></details>
        <details className="lift-accordion"><summary><span>All client programs</span><small>Review or freeze existing plans</small></summary><div className="lift-accordion__content"><div className="lift-suite__programs">{programs.length ? programs.map(program => <article key={program.id}><div><strong>{program.client?.full_name} · {program.title}</strong><span>{program.status}{program.weekly_frequency ? ` · ${program.weekly_frequency}× weekly` : ""}</span>{program.goal ? <p>Goal: {program.goal}</p> : null}</div><button className="lift-preview__small" onClick={() => { setSelectedProgramId(program.id); setWorkoutDay("Day 1"); }}>Open Trainee Card</button></article>) : <p>No programs assigned yet.</p>}</div></div></details>
      </div> : null}
