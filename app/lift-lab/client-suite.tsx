...
type Tab = "home" | "book" | "schedule" | "program" | "payments";
...
type Program = { id: string; title: string; description: string | null; starts_on: string | null; ends_on: string | null; status: string };
type ProgramItem = { id: string; program_id: string; scheduled_for: string | null; exercise_name: string; target_sets: number | null; target_reps: string | null; target_weight: string | null; rest_seconds: number | null; trainer_notes: string | null };
type WorkoutLog = { id: string; program_item_id: string; completed_at: string };
...
export default function LiftLabClientSuite({ userId, firstName }: { userId: string; firstName: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [serviceResult, slotResult, appointmentResult, paymentResult, programResult, itemResult, logResult, noticeResult, settingsResult] = await Promise.all([
      ...
      supabase.from("lift_lab_programs").select("id, title, description, starts_on, ends_on, status").order("created_at", { ascending: false }),
      supabase.from("lift_lab_program_items").select("id, program_id, scheduled_for, exercise_name, target_sets, target_reps, target_weight, rest_seconds, trainer_notes").order("scheduled_for"),
      supabase.from("lift_lab_workout_logs").select("id, program_item_id, completed_at"),
    ...
    const error = [serviceResult, slotResult, appointmentResult, paymentResult, programResult, itemResult, logResult, noticeResult, settingsResult].find(result => result.error)?.error;
  ...
  const tabs: Tab[] = ["home", "book", "schedule", "program", "payments"];
  return <div className="lift-client-suite">
    ...
    {tab === "program" && <div className="lift-suite__stack">{programs.length ? programs.map(program => <section className="lift-lab-card" key={program.id}><p className="eyebrow">{program.status} program</p><h2>{program.title}</h2>{program.description && <p>{program.description}</p>}<div className="lift-client-workouts">{items.filter(item => item.program_id === program.id).map(item => { const log = logs.find(entry => entry.program_item_id === item.id); return <article key={item.id}><div><strong>{item.exercise_name}</strong><span>{item.scheduled_for || "Flexible date"} · {item.target_sets || "—"} sets × {item.target_reps || "—"}{item.target_weight ? ` · ${item.target_weight}` : ""}{item.rest_seconds != null ? ` · ${item.rest_seconds}s rest` : ""}</span>{item.trainer_notes && <p>{item.trainer_notes}</p>}</div>{log ? <span className="lift-lab-status lift-lab-status--completed">Completed</span> : <form className="lift-client-workout-form" onSubmit={event => completeWorkout(event, item)}><input name="sets" type="number" min="0" placeholder="Sets" aria-label="Completed sets" /><input name="reps" placeholder="Reps" aria-label="Completed reps" /><input name="weight" placeholder="Weight" aria-label="Completed weight" /><input name="notes" placeholder="Notes" aria-label="Workout notes" /><button disabled={busy}>Complete</button></form>}</article>; })}</div></section>) : <section className="lift-lab-card lift-lab-empty"><strong>Your program will appear here.</strong><p>Once Madie assigns a workout program, you can track it from this tab.</p></section>}</div>}
