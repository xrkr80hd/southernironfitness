...
type WorkoutLog = { id: string; program_item_id: string; completed_at: string; actual_sets: number | null; actual_reps: string | null; actual_weight: string | null; client_notes: string | null };
...
async function compressImage(file: File) {
  ...
  if (!blob || blob.size > 1_048_576) throw new Error("That image is still too large. Please choose a smaller image.");
...
export default function LiftLabClientSuite({ userId, firstName }: { userId: string; firstName: string }) {
  ...
  const [cropReset, setCropReset] = useState(0);
  ...
  const load = useCallback(async () => {
    ...
    const [serviceResult, slotResult, appointmentResult, paymentResult, programResult, itemResult, logResult, noticeResult, settingsResult, preferenceResult, photoResult] = await Promise.all([
      ...
      supabase.from("lift_lab_workout_logs").select("id, program_item_id, completed_at, actual_sets, actual_reps, actual_weight, client_notes"),
  ...
  async function completeWorkout(event: FormEvent<HTMLFormElement>, item: ProgramItem) {
    ...
    setBusy(true);
    const form = new FormData(event.currentTarget);
    const feeling = String(form.get("feeling") || "As expected");
    const note = String(form.get("notes") || "").trim();
    const { error } = await createClient().from("lift_lab_workout_logs").insert({ program_item_id: item.id, client_id: userId, actual_sets: Number(form.get("sets")) || null, actual_reps: String(form.get("reps") || "") || null, actual_weight: String(form.get("weight") || "") || null, client_notes: `${feeling}${note ? ` — ${note}` : ""}` });
  ...
  async function cancelAppointment(id: string) {
    ...
    setBusy(true);
    const { error } = await createClient().rpc("cancel_lift_lab_appointment", { target_appointment_id: id });
    setMessage(error ? error.message : "Your appointment was cancelled and Madie was notified.");
    if (!error) await load();
    setBusy(false);
  }
  ...
  async function uploadProgressPhoto(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const form = new FormData(event.currentTarget);
      if (!croppedPhoto) throw new Error("Choose a photo and tap Apply Crop first.");
      const file = await compressImage(croppedPhoto);
      ...
      event.currentTarget.reset(); setCroppedPhoto(null); setCropReset(value => value + 1); await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "The photo could not be saved."); }
    setBusy(false);
  }

  async function viewProgressPhoto(photo: ProgressPhoto) {
    setBusy(true);
    const signed = await createClient().storage.from("lift-lab-progress-photos").createSignedUrl(photo.storage_path, 60);
    if (signed.error || !signed.data?.signedUrl) {
      setMessage(signed.error?.message || "Your private photo could not be opened.");
    } else {
      window.open(signed.data.signedUrl, "_blank", "noopener,noreferrer");
      setMessage("Your private photo opened in a secure link that expires in one minute.");
    }
    setBusy(false);
  }
  ...
  return <div className="lift-client-suite">
    <details className="lift-menu-accordion"><summary>Account menu · {tab}</summary><nav className="lift-suite__tabs" aria-label="Lift Lab account sections">{tabs.map(item => <button key={item} className={tab === item ? "is-active" : ""} onClick={() => setTab(item)}>{item}</button>)}</nav></details>
    ...
    {tab === "schedule" && <section className="lift-lab-card"><h2>Your schedule</h2><div className="lift-suite__list">{appointments.length ? appointments.map(item => <article key={item.id}><div><strong>{item.service?.name || "Private session"}</strong><span>{item.availability ? dateTime(item.availability.starts_at) : "Time unavailable"}</span></div><span className={`lift-lab-status lift-lab-status--${item.status}`}>{item.status}</span>{["pending", "confirmed"].includes(item.status) && item.availability && new Date(item.availability.starts_at) > new Date() ? <button className="lift-preview__small" disabled={busy} onClick={() => cancelAppointment(item.id)}>Cancel</button> : null}</article>) : <p>You haven’t requested a session yet.</p>}</div></section>}

    {tab === "program" && <div className="lift-suite__stack lift-admin-accordions">{programs.length ? programs.map(program => { const programItems = items.filter(item => item.program_id === program.id); const days = Array.from(new Set(programItems.map(item => item.day_label || "Flexible day"))); return <details className="lift-accordion lift-accordion--client" key={program.id} open={program.status === "active"}><summary><span>{program.title}</span><small>{program.status}{program.weekly_frequency ? ` · ${program.weekly_frequency} days weekly` : ""}</small></summary><div className="lift-accordion__content">{program.goal && <p><strong>Goal:</strong> {program.goal}</p>}{program.description && <p>{program.description}</p>}{program.status === "frozen" ? <p className="lift-lab-message">This program is frozen. Your history is safe, but new workout logging is paused.</p> : null}<div className="lift-workout-days">{days.length ? days.map(day => { const dayItems = programItems.filter(item => (item.day_label || "Flexible day") === day); const completed = dayItems.filter(item => logs.some(log => log.program_item_id === item.id)).length; return <details className="lift-accordion lift-accordion--day" key={day}><summary><span>{day}</span><small>{completed}/{dayItems.length} exercises completed</small></summary><div className="lift-accordion__content"><div className="lift-client-workouts">{dayItems.map(item => { const log = logs.find(entry => entry.program_item_id === item.id); return <article key={item.id}><div><strong>{item.exercise_name}</strong><span>{item.scheduled_for || "Flexible date"} · {item.target_sets || "—"} sets × {item.target_reps || "—"}{item.target_weight ? ` · ${item.target_weight}` : ""}{item.rest_seconds != null ? ` · ${item.rest_seconds}s rest` : ""}{item.tempo ? ` · ${item.tempo} tempo` : ""}</span>{item.trainer_notes && <p><strong>Madie’s instructions:</strong> {item.trainer_notes}</p>}{log?.client_notes && <p className="lift-client-note"><strong>Your note:</strong> {log.client_notes}</p>}</div>{log ? <span className="lift-lab-status lift-lab-status--completed">Completed</span> : program.status !== "frozen" ? <form className="lift-client-workout-form" onSubmit={event => completeWorkout(event, item)}><div className="lift-client-workout-fields"><input name="sets" type="number" min="0" placeholder="Sets" aria-label="Completed sets" /><input name="reps" placeholder="Reps" aria-label="Completed reps" /><input name="weight" placeholder="Weight" aria-label="Completed weight" /><select name="feeling" aria-label="How the exercise felt" defaultValue="As expected"><option>Too easy</option><option>As expected</option><option>Challenging</option><option>Too difficult</option></select></div><textarea name="notes" rows={4} placeholder="How did it feel? Note form, pain, progress, or anything Madie should know." aria-label="Workout notes" /><button disabled={busy}>Complete Exercise</button></form> : null}</article>})}</div></div></details>; }) : <p>Madie has not added workout days yet.</p>}</div></div></details>; }) : <section className="lift-lab-card lift-lab-empty"><strong>Your program will appear here.</strong><p>Once Madie assigns a workout program, you can track it from this tab.</p></section>}</div>}

    {tab === "progress" && <div className="lift-suite__stack"><div className="lift-suite__metrics"><article><strong>{logs.length}</strong><span>Workouts completed</span></article><article><strong>{appointments.filter(item => ["confirmed", "completed"].includes(item.status)).length}</strong><span>Sessions attended/booked</span></article><article><strong>{programs.find(item => item.status === "active")?.title || "Not assigned"}</strong><span>Current program</span></article></div><section className="lift-lab-card"><h2>Private progress photos</h2><p className="lift-lab-card__intro">Completely optional and off by default. If you opt in, only you and Madie can see these photos. They are never available to Southern Iron Fitness or the master administrator and cannot be used publicly without separate written permission.</p>{!photoPreference?.opted_in ? <div className="lift-photo-consent"><p>Opting in does not change your training agreement. You can turn this off later without affecting your account.</p><button className="button" disabled={busy} onClick={() => setPhotoOptIn(true)}>Opt In to Private Photos</button></div> : <><div className="lift-photo-consent lift-photo-consent--active"><strong>Private photo tracking is on.</strong><button className="lift-lab-text-button" disabled={busy} onClick={() => setPhotoOptIn(false)}>Turn off future photo tracking</button></div><form className="lift-lab-form" onSubmit={uploadProgressPhoto}><label>Progress point<select name="photoType" required defaultValue="check_in"><option value="date_started">Date Started</option><option value="check_in">Visit Check-In</option><option value="six_month">Six Months Progress</option></select></label><label>Date<input name="takenOn" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} /></label><PhotoCropper key={cropReset} onCrop={setCroppedPhoto} /><label>Private note <span>(optional)</span><input name="caption" /></label><button className="button" disabled={busy || !croppedPhoto}>{busy ? "Saving…" : "Save Private Progress Photo"}</button></form><div className="lift-photo-slots">{["date_started", "check_in", "six_month"].map(kind => { const count = photos.filter(photo => photo.photo_type === kind).length; return <article key={kind}><span>{kind.replaceAll("_", " ")}</span><strong>{count ? `${count} saved` : "Not added"}</strong><small>{kind === "six_month" && programs[0]?.starts_on ? `Target ${new Date(new Date(programs[0].starts_on).setMonth(new Date(programs[0].starts_on).getMonth() + 6)).toLocaleDateString()}` : "Optional and private"}</small></article>; })}</div>{photos.length ? <div className="lift-suite__list">{photos.map(photo => <article key={photo.id}><div><strong>{photo.photo_type.replaceAll("_", " ")}</strong><span>{new Date(`${photo.taken_on}T12:00:00`).toLocaleDateString()}{photo.caption ? ` · ${photo.caption}` : ""}</span></div><button className="lift-preview__small" disabled={busy} onClick={() => viewProgressPhoto(photo)}>View Privately</button></article>)}</div> : null}</>}</section></div>}
